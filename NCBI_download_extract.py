...
"""
This program downloads FASTA aminoacid (faa) files for user specified bacteria from NCBI's FTP server and decompresses them. 

This code is optimized for Python 3.6 or later.
"""
...
os.chdir("Treponema_pallidum_faa")
...
for counter, gz_file in enumerate(all_files):
	if ".gz" not in gz_file:
		continue
	faa_file = gz_file.replace(".gz", "")
	print(f"Extracting {gz_file}...")
	with gzip.open(gz_file, 'rb') as f_in:
		with open("faa/" + faa_file, 'wb') as f_out:
			shutil.copyfileobj(f_in, f_out)
	print(f"File {counter + 1} out of {str(len(all_files))} extracted. \n")
	os.remove(gz_file)
